...
Il servizio ID scrive un cookie di prime e terze parti sul browser quando una persona visita il sito (consulta [Cookie e il servizio Experience Cloud ID](../../mcvid-introduction/mcvid-cookies.md)). Il cookie di prima parte contiene il MID, un ID univoco che identifica quel visitatore specifico. Il cookie di terza parte contiene un altro ID usato dal servizio ID per generare il MID. Quando un browser blocca questo cookie di terza parte, il servizio ID non è in grado di:
